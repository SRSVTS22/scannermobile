...
const djangoServerUrl = 'https://smargtechnologies.in:8012/SepioTruCheck/services/decode';
...
function checkGenuineOrFakeProduct(scanText) {
    ...
    const postData = {
        "client_secret_key": "4b180218-6276-11ee-8c99-0242ac120002",
        "uid_url": scanQRText,
    ...
    fetch(djangoServerUrl, requestOptions)
        ...
        .then((data) => {
            ...
            let genuineOrfakeSign = document.getElementById('productGenuineOrFakeSign');
            genuineOrfakeSign.style.display = 'block'
            genuineOrfakeSign.style.color = '#ff3737';
            genuineOrfakeSign.innerText = "Result : " + data.resultdata.message;
...
function dummydata() {
    ...
    //alert("Scanned Data from QR :"+"\n\n Mask: "+finalMask+"\n Version No: "+finalVersionNo+"\n ECC: "+ECC_Level+"\n"+"QR Text: "+scanQRText+"\n Codeword :"+finalCodewordList+"\n\n JSON Data \n"+"\n Mask: "+scanTxtLQ[0]['uid_mask']+"\nVersion: "+scanTxtLQ[0]['uid_version']+"\nECC: "+scanTxtLQ[0]['uid_ecc']+"\nQR Text: "+scanTxtLQ[0]['uid_url']+"\n Count: "+countW+"\n Position 1:"+scanTxtLQ[0]['position_1']+"\n Position 2:"+scanTxtLQ[0]['position_2']+"\n Position 3:"+scanTxtLQ[0]['position_3']+"\n Position 4:"+scanTxtLQ[0]['position_4'])

    //[DRS] Checking genuine and fake product
    let finalCodewordList = (Final_CodeWord != null && Final_CodeWord != undefined && Final_CodeWord != '') ? Final_CodeWord.split(',') : [];
    let scanTxtLQ = (productAuthList != null && productAuthList != undefined && productAuthList.length != 0) ? productAuthList.filter((item) => { return item['uid_url'] == scanQRText }) : [];
    // alert("Decode Text:"+scanQRText) // [DRS] If Higher level ECC apply then decoded text mismatch 
    if (finalCodewordList != null && finalCodewordList != undefined && finalCodewordList.length != 0 && scanTxtLQ.length != 0) {
        ...
        // let extraTxt = null;
        // alert(scanQRText)
        // alert("UID Mask: "+scanTxtLQ[0]['uid_mask'])
        // alert("UID Version: "+scanTxtLQ[0]['uid_version'])
        // alert("UID ECC: "+scanTxtLQ[0]['uid_ecc'])
        // alert("UID position_1: "+scanTxtLQ[0]['position_1'])
        // alert("UID position_1: "+scanTxtLQ[0]['position_2'])
        // alert("UID position_3: "+scanTxtLQ[0]['position_3'])
        // alert("UID position_4: "+scanTxtLQ[0]['position_4'])

        // alert("Decode Final Version: "+finalVersionNo)
        // alert("Decode final Mask: "+finalMask)
        // alert("Decode ECC Level: "+ECC_Level)
        // alert("Decode Codeword: "+finalCodewordList)

        if (finalVersionNo == 3 && finalMask == scanTxtLQ[0]['uid_mask'] && finalVersionNo == scanTxtLQ[0]['uid_version'] && eccLevel[scanTxtLQ[0]['uid_ecc']] == ECC_Level) {
            cdOne = (scanTxtLQ[0]['position_1'] != finalCodewordList[VERSION_3_POS_1]);
            cdTwo = (scanTxtLQ[0]['position_2'] != finalCodewordList[VERSION_3_POS_2]);
            cdThree = (scanTxtLQ[0]['position_3'] != finalCodewordList[VERSION_3_POS_3]);
            cdFour = (scanTxtLQ[0]['position_4'] != finalCodewordList[VERSION_3_POS_4]);
            // extraTxt = "Position 1: "+"JSON :"+scanTxtLQ[0]['position_1']+" Decode : "+finalCodewordList[VERSION_3_POS_1]+"\n"+"Position 2: "+"JSON :"+scanTxtLQ[0]['position_2']+"Decode :"+finalCodewordList[VERSION_3_POS_2]+"\n"+"Position 3: "+"JSON :"+scanTxtLQ[0]['position_3']+"Decode :"+finalCodewordList[VERSION_3_POS_3]+"\n"+"Position 4: "+"JSON :"+scanTxtLQ[0]['position_4']+"Decode :"+finalCodewordList[VERSION_3_POS_4];
            // alert(extraTxt)
        } else if (finalVersionNo == 4 && finalMask == scanTxtLQ[0]['uid_mask'] && finalVersionNo == scanTxtLQ[0]['uid_version'] && eccLevel[scanTxtLQ[0]['uid_ecc']] == ECC_Level) {
            cdOne = (scanTxtLQ[0]['position_1'] != finalCodewordList[VERSION_4_POS_1]);
            cdTwo = (scanTxtLQ[0]['position_2'] != finalCodewordList[VERSION_4_POS_2]);
            cdThree = (scanTxtLQ[0]['position_3'] != finalCodewordList[VERSION_4_POS_3]);
            cdFour = (scanTxtLQ[0]['position_4'] != finalCodewordList[VERSION_4_POS_4]);
            // extraTxt = "Position 1: "+"JSON :"+scanTxtLQ[0]['position_1']+" Decode : "+finalCodewordList[VERSION_4_POS_1]+"\n"+"Position 2: "+"JSON :"+scanTxtLQ[0]['position_2']+"Decode :"+finalCodewordList[VERSION_4_POS_2]+"\n"+"Position 3: "+"JSON :"+scanTxtLQ[0]['position_3']+"Decode :"+finalCodewordList[VERSION_4_POS_3]+"\n"+"Position 4: "+"JSON :"+scanTxtLQ[0]['position_4']+"Decode :"+finalCodewordList[VERSION_4_POS_4];
            // alert(extraTxt)
        } else if (finalVersionNo == 5 && finalMask == scanTxtLQ[0]['uid_mask'] && finalVersionNo == scanTxtLQ[0]['uid_version'] && eccLevel[scanTxtLQ[0]['uid_ecc']] == ECC_Level) {
            cdOne = (scanTxtLQ[0]['position_1'] != finalCodewordList[VERSION_5_POS_1]);
            cdTwo = (scanTxtLQ[0]['position_2'] != finalCodewordList[VERSION_5_POS_2]);
            cdThree = (scanTxtLQ[0]['position_3'] != finalCodewordList[VERSION_5_POS_3]);
            cdFour = (scanTxtLQ[0]['position_4'] != finalCodewordList[VERSION_5_POS_4]);
            // extraTxt = "Position 1: "+"JSON :"+scanTxtLQ[0]['position_1']+" Decode : "+finalCodewordList[VERSION_5_POS_1]+"\n"+"Position 2: "+"JSON :"+scanTxtLQ[0]['position_2']+"Decode :"+finalCodewordList[VERSION_5_POS_2]+"\n"+"Position 3: "+"JSON :"+scanTxtLQ[0]['position_3']+"Decode :"+finalCodewordList[VERSION_5_POS_3]+"\n"+"Position 4: "+"JSON :"+scanTxtLQ[0]['position_4']+"Decode :"+finalCodewordList[VERSION_5_POS_4];
            // alert(extraTxt)
        }
        ...
        // [DRS] Check the conditaion match count out of 4 and below two rows will show Alert for scanned and JSON Data

        if (countW > 2) {

            let SampleText = "Scanned Data from QR :" + "\n\n Mask: " + finalMask + "\n Version No: " + finalVersionNo + "\n ECC: " + ECC_Level + "\n" + "QR Text: " + scanQRText + "\n Codeword :" + finalCodewordList + "\n\n JSON Data \n" + "\n Mask: " + scanTxtLQ[0]['uid_mask'] + "\nVersion: " + scanTxtLQ[0]['uid_version'] + "\nECC: " + scanTxtLQ[0]['uid_ecc'] + "\nQR Text: " + scanTxtLQ[0]['uid_url'] + "\n Count: " + countW + "\n Position 1:" + scanTxtLQ[0]['position_1'] + "\n Position 2:" + scanTxtLQ[0]['position_2'] + "\n Position 3:" + scanTxtLQ[0]['position_3'] + "\n Position 4:" + scanTxtLQ[0]['position_4'];
            //alert(SampleText)
            ...
            genuineOrfake.style.color = '#008000';
            genuineOrfake.innerText = "Result: Genuine";
            let genuineOrfakeSign = document.getElementById('productGenuineOrFakeSign');
            genuineOrfakeSign.style.display = 'none'
            $('#exampleModal').modal('show');
            ...
            genuineOrfake.innerText = "Result: Fake"
            let genuineOrfakeSign = document.getElementById('productGenuineOrFakeSign');
            genuineOrfakeSign.style.display = 'block'
            genuineOrfakeSign.style.color = '#ff3737';
            genuineOrfakeSign.innerText = "(Signature Mismatch)";
            $('#exampleModal').modal('show');
        }
        isGenuineOrFakeModalOpen = true;
    } else {

        // alert(scanQRText== undefined || scanQRText.length == 0 ? 'Scan Text Not Match' :  'Scan Text Match')
        let SampleText = "Scanned Data from QR :" + "\n\n Mask: " + finalMask + "\n Version No: " + finalVersionNo + "\n ECC: " + ECC_Level + "\n" + "QR Text: " + scanQRText + "\n\n\n Codeword :" + finalCodewordList;
        //alert(SampleText)
...
//[DRS] Get uploaded json file for check qr genuine or fake
function getJSONFileDataList() {
    ///[DRS] Change if you want deploy other server to change file name sample.json to actual file name
    fetch('https://sepio-test.s3.ap-south-1.amazonaws.com/2FA/SRS_Demo.json').then(function (response) {
        return response.json();
    }).then(function (data) {
        console.log(data);
        productAuthList = data;
    }).catch(function (err) {
